...
?>
 <form method="post" action="<?= admin_url('client/save_client'); ?>">
    ...
    if (!in_array('company', $patient_inactive_fields) || !in_array('salutation', $patient_inactive_fields) || !in_array('gender', $patient_inactive_fields)) {
        ...
        <?php
			// Capture any buffered field content
			$fields[] = ob_get_clean();
	}
			// Map of field name => field HTML
			$fields_map = [

				'enquiry_type_id' => render_select(
					'enquiry_type_id',
					$enquiry_type,
					['enquiry_type_id', ['enquiry_type_name']],
					_l('enquiry_type') . ' *',
					!empty($patient['enquiry_type_id']) 
						? $patient['enquiry_type_id'] 
						: ($enquiry_type[0]['enquiry_type_id'] ?? ''),
				...
				'contact_number' => '<label class="form-label">' . _l('contact_number') . ' *</label>
					...
					value="' . htmlspecialchars($patient['phonenumber'] ?? $this->input->post('contact_number')) . '" required >',
