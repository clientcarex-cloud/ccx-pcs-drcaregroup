...
return App_table::find('clients')
    ->outputUsing(function ($params) {
        ...
        $hasPermissionDelete = staff_can('delete', 'patients');
        ...
        $aColumns = [
			...
			'city',
			'state',
			'(SELECT short_name FROM ' . db_prefix() . 'countries WHERE ' . db_prefix() . 'countries.country_id = ' . db_prefix() . 'clients.country LIMIT 1) as country',
			'(SELECT CONCAT(firstname, " ", lastname)
			  FROM ' . db_prefix() . 'staff 
			  JOIN ' . db_prefix() . 'appointment 
				ON ' . db_prefix() . 'appointment.enquiry_doctor_id = ' . db_prefix() . 'staff.staffid 
			  WHERE ' . db_prefix() . 'appointment.userid = ' . db_prefix() . 'clients.userid 
			  LIMIT 1) as enquiry_doctor',
			'(SELECT CONCAT(firstname, " ", lastname)
				FROM ' . db_prefix() . 'staff 
				JOIN ' . db_prefix() . 'appointment 
				ON ' . db_prefix() . 'appointment.unit_doctor_id = ' . db_prefix() . 'staff.staffid 
				 WHERE ' . db_prefix() . 'appointment.userid = ' . db_prefix() . 'clients.userid
				LIMIT 1) as unit_doctor',
			'(SELECT patient_response_name
				FROM ' . db_prefix() . 'patient_response 
				JOIN ' . db_prefix() . 'appointment 
				ON ' . db_prefix() . 'appointment.patient_response_id = ' . db_prefix() . 'patient_response.patient_response_id 
				 WHERE ' . db_prefix() . 'appointment.userid = ' . db_prefix() . 'clients.userid
				LIMIT 1) as patient_response',
        ...
        foreach ($rResult as $aRow) {
            ...
            $company  .= '<br><label style="font-weight: 300; font-size: 12px">'.e(_dt($aRow['datecreated'])).'</label>';
            ...
            if ($hasPermissionDelete) {
				$company .= '<a href="' . admin_url('clients/delete/' . $aRow['userid']) . '" class="_delete" onclick="return confirm(\'Are you sure you want to delete this record?\')">' . _l('delete') . '</a>';
			}
    ...
    <label class="onoffswitch-label" for="' . $aRow['userid'] . '"></label>
			...
			//$row[] = e($aRow['patient_response']);
			
			$row[] = '<a href="' . admin_url('client/index/' . $aRow['userid']) . '" data-toggle="tooltip" title="' . _l('view') . '">'
    . e($aRow['patient_response']) . '</a>';
        ...
        App_table_filter::new('customer_admins', 'MultiSelectRule')->label(_l('responsible_admin'))
            ->isVisible(fn () => staff_can('create', 'patients') || staff_can('edit', 'patients'))
            ->options(function ($ci) {
