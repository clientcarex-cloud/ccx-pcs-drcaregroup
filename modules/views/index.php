            ...
            //SMS Balance Checker and passing to Numbers Validation JS
            
                 $this->db->select('ccredit');
                 $this->db->order_by('id','desc');
                 $this->db->limit(1);
                 $query = $this->db->get(db_prefix().'_smscredits');
                 $smsbalance = $query->result();
                 
                 $smsbalance = $smsbalance[0]->ccredit;
                 echo "<script> var smsbalance = ". json_encode($smsbalance) ."</script>";
                 ?>
             <style>
