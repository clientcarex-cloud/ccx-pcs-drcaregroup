        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="form-group">
                    <label for="created_at"><?= _l('date_created'); ?></label>
                    <input name="created_at" class="form-control" type="datetime" value="<?= date('Y-m-d H:i:s'); ?>" />
                </div>

            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-default" data-dismiss="modal"><?php echo _l('close'); ?></button>
                <button type="button" class="btn btn-primary assign-client-affiliate"
                    onclick="affiliateManagementAssign()"><?= _l('submit'); ?></button>
            </div>
        </div><!-- /.modal-content -->
    </div><!-- /.modal-dialog -->
</div><!-- /.modal -->
...
"use strict";
$(function() {
    initDataTable('.table-referrals', window.location.href, undefined, [], undefined, [
        <?= count($columns) - 1; ?>,
        'desc'
    ]);
});
...
function affiliateManagementAssign() {
    ...
    let data = {
        ...
        created_at: $("#add_referral_modal input[name=created_at]").val(),
    };
    console.log(data);
